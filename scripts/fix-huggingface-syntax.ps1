...
# Create a fixed version with proper template literals and escaping
$fixed = @'
...
Set-Content -Path $servicePath -Value $fixed

Write-Host "Fixed syntax issues in $servicePath" -ForegroundColor Green
Write-Host "`nYour Hugging Face integration is now ready to use!" -ForegroundColor Green
Write-Host "You can test it by running the app with: npm start" -ForegroundColor Yellow
